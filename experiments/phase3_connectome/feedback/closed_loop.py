...
OUT = ROOT / "benchmarks" / "closed_loop_feedback_result.json"
...
dynamics = load_module(
    ...
    "phase3_closed_loop_dynamics",
)

adapter = load_module(
    SIMULATOR / "connectome_adapter.py",
    "phase3_closed_loop_adapter",
)

feedback = load_module(
    ROOT / "feedback" / "controller.py",
    "phase3_closed_loop_feedback",
)
...
def run(steps=12):
    ...
    for step in range(steps):
        ...
        # Feed the bounded controller correction back only into
        # the deterministic seed channel.
        correction = fb["correction"]
    ...
    result = {
        "experiment": "phase3_connectome_closed_loop_feedback",
        "model": "experimental_computational_model",
        "nodes": len(circuit.nodes),
        "edges": len(circuit.edges),
        "steps": steps,
        "target_activity": controller.config.target_activity,
        "metrics": {
            ...
            "max_abs_correction": max(abs(x) for x in corrections),
        },
        "history": history,
    }
...
if __name__ == "__main__":
    ...
    print("Phase 3 closed-loop feedback benchmark complete")
    ...
    print("Mean activity:", result["metrics"]["mean_activity"])
    ...
    print("Max correction:", result["metrics"]["max_abs_correction"])
    print("Result:", OUT)
