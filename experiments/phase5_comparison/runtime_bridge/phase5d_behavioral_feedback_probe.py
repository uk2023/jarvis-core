...
CASES = [
    ...
    },
    {
        ...
        "expected": "NEURAL-BEHAVIOR-5D",
        "workflow_signal": 0.50,
        "expected_thinking": True,
    },
    {
        "id": "T03",
        "name": "reasoning_behavior",
        "turns": [
...
def usage_delta(before: Dict[str, Any], after: Dict[str, Any]) -> Dict[str, int]:
    ...
    return {
        "llm_calls": max(0, int(a.get("calls", 0) or 0) - int(b.get("calls", 0) or 0)),
        "prompt_tokens": max(0, int(a.get("prompt_tokens", 0) or 0) - int(b.get("prompt_tokens", 0) or 0)),
        "completion_tokens": max(0, int(a.get("completion_tokens", 0) or 0) - int(b.get("completion_tokens", 0) or 0)),
        "total_tokens": max(0, int(a.get("total_tokens", 0) or 0) - int(b.get("total_tokens", 0) or 0)),
    }
...
def run_isolated(case: Dict[str, Any], neural_enabled: bool, trial: int, variant: str) -> Dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="phase5d_") as tmp:
        ...
        proc = ctx.Process(target=isolated_worker, args=(case, neural_enabled, trial, variant, str(result_path)))
        proc.start()
        proc.join()
...
def aggregate(result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    thinking_matches = [bool(x.get("thinking_used")) == bool(expected) for x in behavior] if expected is not None else []
    ...
    return {
        ...
        "thinking_expectation_matches": sum(thinking_matches) if thinking_matches else None,
        "thinking_expectation_total": len(thinking_matches) if thinking_matches else None,
...
def main() -> None:
    ...
    payload: Dict[str, Any] = {
        ...
        "metrics_source": "deep_inspector._real_turn_metrics + persistent token ledger delta",
        "llm_usage_rule": "Only persistent successful provider response usage is counted; no token estimates are used.",
        "results": [],
    }

    for case in CASES:
        case_entry = {"id": case["id"], "name": case["name"], "expected_thinking": case.get("expected_thinking"), "trials": []}
        for trial in range(1, trials + 1):
            if trial % 2:
                a = run_isolated(case, False, trial, "A")
                b = run_isolated(case, True, trial, "B")
            else:
                b = run_isolated(case, True, trial, "B")
                a = run_isolated(case, False, trial, "A")
            a["expected_thinking"] = case.get("expected_thinking")
            b["expected_thinking"] = case.get("expected_thinking")
            aagg = aggregate(a)
            bagg = aggregate(b)
            case_entry["trials"].append({
                ...
                "B_aggregate": bagg,
                "behavioral_delta": {
                    "thinking_used_delta_B_minus_A": bagg["thinking_used_turns"] - aagg["thinking_used_turns"],
                    "thinking_stage_delta_B_minus_A": bagg["thinking_stage_count"] - aagg["thinking_stage_count"],
                    "llm_calls_delta_B_minus_A": bagg["llm_calls"] - aagg["llm_calls"],
                    "total_tokens_delta_B_minus_A": bagg["total_tokens"] - aagg["total_tokens"],
                    "latency_delta_B_minus_A_ms": bagg["mean_latency_ms"] - aagg["mean_latency_ms"],
                },
            })
            print(
                f"[{case['id']}] trial {trial}: "
                f"thinking A/B={aagg['thinking_used_turns']}/{bagg['thinking_used_turns']} "
                f"LLM calls A/B={aagg['llm_calls']}/{bagg['llm_calls']} "
                f"tokens A/B={aagg['total_tokens']}/{bagg['total_tokens']} "
                f"latency delta={bagg['mean_latency_ms']-aagg['mean_latency_ms']:+.2f} ms"
            )
