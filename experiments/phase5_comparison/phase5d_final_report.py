...
def main() -> None:
    ...
    latency_deltas, provider_latency_deltas, call_deltas, token_deltas = [], [], [], []
    ...
    lines += [
        ...
        "DECISION GATE", "------------",
        "Phase 5D measures whether bounded neural feedback changes actual Brain thinking behavior and what real provider usage it causes.",
        "LLM calls/tokens are counted only from the persistent real-usage ledger; local token estimates are excluded.",
        "Provider response latency is reported only when the real provider telemetry publishes it; otherwise it remains zero/not published rather than estimated.",
